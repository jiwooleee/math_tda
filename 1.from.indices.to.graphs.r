...
for(i in 1:length(novels)){
  ...
  # tolower() transforms texts to lowercases
...
if (length(useless.list) > 0){
  
  print("There are useless index files.")
  
  for (i in length(useless.list):1){
    
    # error: object delete.list not found
    for.delete <- useless.list[[i]]  
