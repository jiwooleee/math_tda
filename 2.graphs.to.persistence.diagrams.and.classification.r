...
my_wd <- "/Users/ljw12/Desktop/TDA/Novels_TDA"
...
#*****FIXED*****
# error: detach invalid 'name' argument
...
if("package:transport" %in% search()) detach("package:transport", unload=TRUE) 
...
accuracy.function <- function(writer1 , writer2 , random_name){
  ...
  for (i in 1:(length(files)) ){
    ...
    # *****FIXED***** 
    # Error in ripsDiag(X = novelsRd_t.0[[i]], maxdimension, maxscale, dist = "arbitrary",  : 
    # there is a bug for computing homological features of dimension higher than 1 when the distance is arbitrary and library 'GUDHI' is used
    # *****https://www.rdocumentation.org/packages/TDA/versions/1.7.5/topics/ripsDiag*****
...
#error: $<-.data.frame`(`*tmp*`, count, value = 1) : replacement has 1 row, data has 0

results1 <- aggregate(count~novel , data=results , FUN=sum)

#error: Error in eval(predvars, data, env) : object 'count' not found
results$count <- NULL

results$correct <- ifelse(results$writer == results$prediction , 1 , 0)

# error: prediction, invalid argument to unary operator
results2 <- aggregate(correct ~. , data=results[,-'prediction'] , FUN=sum)

# error: results1 not found
results.summary <- merge(results1,results2 , by='novel')

# error: object results.summary not found
